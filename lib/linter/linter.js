...
function runRules(
	...
	for (const ruleId in configuredRules) {
		...
		if (timing.enabled || stats) {
			 ...
			 * pass raw listener functions directly to the visitor with
			 * the ruleId for error attribution. The visitor's callSync
			 * and callSyncSingle handle try-catch internally, eliminating
			 * the need for wrapper closures (~one closure per rule per
			 * selector saved).
			 */
			for (const selector in ruleListeners) {
				if (!Object.hasOwn(ruleListeners, selector)) {
				...
				visitor.add(selector, ruleListeners[selector], ruleId);
			}
		}
	}
