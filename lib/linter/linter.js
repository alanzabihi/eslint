...
function runRules(
	...
	/*
	 * Set parent properties on AST nodes before rule creation so that
	 * rules can access node.parent in their create() functions.
	 * This is a lightweight walk that only assigns parent pointers.
	 */
	if (typeof sourceCode.setParentProperties === "function") {
		sourceCode.setParentProperties();
	}

	for (const ruleId in configuredRules) {
		if (!Object.hasOwn(configuredRules, ruleId)) {
			continue;
		}
		...
		if (timing.enabled || stats) {
			for (const selector in ruleListeners) {
				if (!Object.hasOwn(ruleListeners, selector)) {
					continue;
				}
				const ruleListener = timing.time(
					ruleId,
					ruleListeners[selector],
					stats,
				);

				visitor.add(selector, (...listenerArgs) => {
					try {
						const ruleListenerReturn = ruleListener(
							...listenerArgs,
						);
			...
			/*
			 * Fast path: when timing/stats are disabled (common case),
			 * use a simpler wrapper that avoids rest/spread overhead.
			 * Listeners receive a single node argument via callSyncSingle,
			 * so the direct single-arg call avoids array packing/unpacking.
			 */
			for (const selector in ruleListeners) {
				if (!Object.hasOwn(ruleListeners, selector)) {
					continue;
				...
				visitor.add(selector, (a, b, c) => {
					try {
						return ruleListener(a, b, c);
					} catch (e) {
						e.ruleId = ruleId;
						throw e;
					}
