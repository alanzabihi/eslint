	...
	calculateSelectors(node, ancestry, isExit) {
		...
		while (
			...
			if (!hasMoreNodeTypeSelectors || isAnyTypeSelectorLessSpecific) {
				...
				const fc = anyTypeSelector.fastCheck;
				...
				const fc = nodeTypeSelector.fastCheck;
			...
			switch (step.kind) {
				case STEP_KIND_VISIT: {
					try {
						...
						if (step.phase === 1) {
							const selectors = esquery.calculateSelectors(
								node,
								currentAncestry,
								false,
							);
							...
							for (let si = 0; si < selectors.length; si++) {
								visitor.callSyncSingle(selectors[si], node);
							}
