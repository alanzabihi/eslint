...
class ESQueryHelper {
	...
	dispatchSelectors(node, ancestry, isExit, visitor, stepArgs, stepTarget) {
		...
		for (let i = 0; i < selectorList.length; i++) {
			...
			/*
			 * Fast paths (checked in order of likelihood):
			 *  1. Simple identifier selectors (e.g. "FunctionDeclaration") —
			 *     guaranteed to match any node of this type.
			 *  2. fastCheck === true — all-identifier "matches" selector
			 *     (e.g. "Identifier, JSXIdentifier") — guaranteed match.
			 *  3. fastCheck is a function — compiled attribute check
			 *     (e.g. "Literal[regex]" compiled to node => node.regex != null).
			 *  4. Fall back to full esquery.matches().
			 */
			if (selector.root.type === "identifier") {
				// guaranteed match — dispatch immediately
			} else {
				const fc = selector.fastCheck;
