	...
	traverseSync(sourceCode, visitor, { steps } = {}) {
		...
		for (let i = 0; i < allSteps.length; i++) {
			...
			if (step.kind === STEP_KIND_VISIT) {
				if (step.phase === 1) {
					/*
					 * Leaf-node optimisation: if the very next step is the
					 * matching exit for the same AST node, no children were
					 * visited in between. We can dispatch both enter and exit
					 * selectors without pushing/popping the ancestry array,
					 * saving O(depth) element moves per leaf.
					 */
					const next = allSteps[i + 1];

					if (
						next &&
						next.kind === STEP_KIND_VISIT &&
						next.phase === 2 &&
			...
			} else if (step.kind === STEP_KIND_CALL) {
