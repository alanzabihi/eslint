...
class ESQueryHelper {
	...
	dispatchSelectors(node, ancestry, isExit, visitor, stepArgs, stepTarget) {
		...
		for (let i = 0; i < selectorList.length; i++) {
			 ...
			 * For all other selectors, run the full match. Ancestry is stored
			 * in push/pop order (parent at end), so reverse it lazily for
			 * esquery which expects parent at index 0.
			 */
			if (selector.root.type === "identifier") {
				...
				if (!reversedAncestry) {
					...
					for (let j = ancestry.length - 1; j >= 0; j--) {
						reversedAncestry.push(ancestry[j]);
					}
				}
				if (
					matches(
				...
				case STEP_KIND_VISIT: {
					try {
						if (step.phase === 1) {
							...
							currentAncestry.push(step.target);
						} else {
							currentAncestry.pop();
							esquery.dispatchSelectors(
								step.target,
