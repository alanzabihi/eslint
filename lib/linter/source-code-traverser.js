...
class SourceCodeTraverser {
	...
	traverseSync(sourceCode, visitor, { steps } = {}) {
		...
		/*
		 * If traverseInline is available and no pre-built steps were passed,
		 * dispatch events directly during the AST walk instead of
		 * materializing a step array. This eliminates 193K object
		 * allocations and avoids a separate parent-property walk.
		 */
		if (!steps && typeof sourceCode.traverseInline === "function") {
			sourceCode.traverseInline({
				onVisitEnter(node) {
					try {
				...
				onCallMethod(name, args) {
					visitor.callSync(name, ...args);
				},
			});
			return;
		}
