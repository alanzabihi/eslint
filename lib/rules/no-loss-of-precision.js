...
const NON_BASE_TEN_PREFIX_RE = /^0[xbo]/iu;
const LEGACY_OCTAL_RE = /^0[0-7]+$/u;
const NUMERIC_SEPARATOR_RE = /_/gu;
...
function getRaw(node) {
	return node.raw.replace(NUMERIC_SEPARATOR_RE, "");
}
...
function isBaseTen(node) {
	return (
		!NON_BASE_TEN_PREFIX_RE.test(node.raw) &&
		!LEGACY_OCTAL_RE.test(node.raw)
	);
}
	...
	create(context) {
		return {
			Literal(node) {
				...
				/*
				 * Safe integers written as integer literals are exactly representable.
				 * Skip the fast path for non-integer syntax (decimals, exponents)
				 * since e.g. 1.0000000000000000000000123 parses to 1 (a safe
				 * integer) but the literal itself loses precision.
				 */
				if (
