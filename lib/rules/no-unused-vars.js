...
module.exports = {
	...
	create(context) {
		...
		function isUsedVariable(variable) {
			...
			/*
			 * Lazy function-definition detection:
			 * -1 = not checked, 0 = no func defs, 1 = has func defs
			 */
			let funcDefState = -1;
			let functionNodes = null;
			let rhsNode = null;

			for (let ri = 0; ri < refsLen; ri++) {
				const ref = refs[ri];
				...
				if (ref.isRead() && !forItself) {
					// First qualifying read: check if variable is a function definition
					if (funcDefState === -1) {
						funcDefState = 0;
					...
					// Non-function variable: this read is genuine
					if (funcDefState === 0) {
						return true;
					}

					// Function variable: build node list once, check self-reference
					if (functionNodes === null) {
						functionNodes = [];
						const defs = variable.defs;

						for (let d = 0, dl = defs.length; d < dl; d++) {
							const { type, node } = defs[d];
							...
							if (type === "FunctionName") {
								...
								functionNodes.push(node.init);
							}
						}
					}

					if (!isSelfReference(ref, functionNodes)) {
						return true;
					}
				}
			}
		...
		function isAfterLastUsedArg(variable) {
			...
			for (let i = params.indexOf(variable) + 1; i < params.length; i++) {
				const v = params[i];

				if (v.references.length > 0 || v.eslintUsed) {
					return false;
				}
			}
			return true;
		 ...
		 * @param {Scope} rootScope an eslint-scope Scope object.
		 * @param {Variable[]} unusedVars an array that saving result.
		 * @returns {Variable[]} unused variables of the scope and descendant scopes.
		 * @private
		 */
		function collectUnusedVariables(rootScope, unusedVars) {
			...
			while (stack.length > 0) {
				...
				if (scope.type !== "global" || config.vars === "all") {
					for (let i = 0, l = variables.length; i < l; ++i) {
						...
						if (def) {
							const type = def.type;
							let refUsedInArrayPatterns = false;
							...
							if (
								...
								for (let r = 0, rl = refs.length; r < rl; r++) {
									if (
										refs[r].identifier.parent.type ===
										"ArrayPattern"
									) {
				...
				// Push child scopes in reverse order for DFS traversal
				const childScopes = scope.childScopes;

				for (let i = childScopes.length - 1; i >= 0; --i) {
					stack.push(childScopes[i]);
				}
			}
