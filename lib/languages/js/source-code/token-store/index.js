...
module.exports = class TokenStore {
	...
	constructor(tokens, comments) {
		...
		// Mutable wrapper survives Object.freeze(this) since freeze is shallow.
		this[INDEX_MAP] = { map: null, lookups: 0, tokens, comments };
	}

	/**
	 * Returns the index map, lazily building it after a lookup threshold.
	 * @returns {Object|null} The hash map or null for binary search fallback.
	 * @private
	 */
	_getIndexMap() {
		const holder = this[INDEX_MAP];

		if (holder.map) {
			return holder.map;
		}
		if (++holder.lookups >= LOOKUP_THRESHOLD) {
			holder.map = createIndexMap(holder.tokens, holder.comments);
			return holder.map;
		}
		return null;
	}
