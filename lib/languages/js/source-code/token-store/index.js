...
const LOOKUP_THRESHOLD = 200;
const lazyIndexMaps = new WeakMap();
const lookupCounts = new WeakMap();
...
module.exports = class TokenStore {
	...
	/**
	 * Returns the index map, lazily building it after a lookup threshold.
	 * Uses WeakMaps since SourceCode instances are frozen.
	 * @returns {Object|null} The index map or null for binary search fallback.
	 * @private
	 */
	_getIndexMap() {
		const cached = lazyIndexMaps.get(this);

		if (cached) {
			return cached;
		}

		const count = (lookupCounts.get(this) || 0) + 1;

		lookupCounts.set(this, count);
		if (count >= LOOKUP_THRESHOLD) {
			const map = createIndexMap(this[TOKENS], this[COMMENTS]);

			lazyIndexMaps.set(this, map);
			return map;
		}
		return null;
	}
