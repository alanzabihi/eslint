...
class SourceCode extends TokenStore {
	...
	/**
	 * Cached merged tokens and comments array.
	 * @type {Token[]|null}
	 */
	#mergedTokensCache = null;

	/**
	 * Tokens for deferred sorted merge.
	 * @type {Token[]}
	 */
	#mergedTokensTokens;

	/**
	 * Comments for deferred sorted merge.
	 * @type {Comment[]}
	 */
	#mergedTokensComments;
	...
	constructor(textOrConfig, astIfNoConfig) {
		...
		this.#mergedTokensTokens = ast.tokens;
		this.#mergedTokensComments = ast.comments;
	...
	/**
	 * Gets the merged tokens and comments array, building it lazily on first access.
	 * @type {Token[]}
	 */
	get tokensAndComments() {
		if (this.#mergedTokensCache === null) {
			this.#mergedTokensCache = sortedMerge(
				this.#mergedTokensTokens,
				this.#mergedTokensComments,
			);
		}
		return this.#mergedTokensCache;
	}
	...
	traverse() {
		...
		let analyzer = {
			enterNode(node) {
				steps.push({ kind: 1, target: node, phase: 1 });
			},
			leaveNode(node) {
				steps.push({ kind: 1, target: node, phase: 2 });
			},
			emit(eventName, a, b, c) {
				steps.push({ kind: 2, target: eventName, a, b, c });
			},
		};
