...
const TokenStore = require("./token-store"),
	...
	{ ConfigCommentParser, Directive } = require("@eslint/plugin-kit"),
...
class SourceCode extends TokenStore {
	...
	constructor(textOrConfig, astIfNoConfig) {
		...
		const astTokensRef = ast.tokens;
		const astCommentsRef = ast.comments;
		const cachesRef = this[caches];

		Object.defineProperty(this, "tokensAndComments", {
			get() {
				let merged = cachesRef.get("tokensAndComments");

				if (!merged) {
					merged = sortedMerge(astTokensRef, astCommentsRef);
					cachesRef.set("tokensAndComments", merged);
				}
	...
	traverse() {
		...
		let analyzer = {
			enterNode(node) {
				steps.push({ kind: 1, target: node, phase: 1, args: null });
			},
			leaveNode(node) {
				steps.push({ kind: 1, target: node, phase: 2, args: null });
			},
			emit(eventName, emitArgs) {
				steps.push({ kind: 2, target: eventName, args: emitArgs });
			},
		};
	...
	/**
	 * Traverse the source code inline, dispatching events directly during
	 * the AST walk instead of building a step array. Assumes parent
	 * properties are already set on AST nodes.
	 * @param {Object} handler Handler with onVisitEnter, onVisitLeave, onCallMethod.
	 * @returns {void}
	 */
	traverseInline(handler) {
		let analyzer = {
			enterNode(node) {
				handler.onVisitEnter(node);
			},
			leaveNode(node) {
				handler.onVisitLeave(node);
			},
			emit(eventName, emitArgs) {
				handler.onCallMethod(eventName, emitArgs);
			},
		};
		...
		/**
		 * Lean recursive walk — avoids Traverser class overhead
		 * (no object allocation, no skip/break checks, no current tracking).
		...
		function walk(node) {
			...
			const keys = visitorKeys[node.type] || vk.getKeys(node);
	...
	/**
	 * Set parent properties on all AST nodes. This is a lightweight
	 * walk that only assigns node.parent without building any data structures.
	 * @returns {void}
	 */
	setParentProperties() {
		const visitorKeys = this.visitorKeys;
