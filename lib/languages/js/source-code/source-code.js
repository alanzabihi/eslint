...
class SourceCode extends TokenStore {
	...
	constructor(textOrConfig, astIfNoConfig) {
...

		{
			...
			Object.defineProperty(this, "tokensAndComments", {
				get() {
					if (cached === null) {
						cached = sortedMerge(astTokensRef, astCommentsRef);
					}
					return cached;
				},
				enumerable: true,
	...
	traverse() {
		...
		let analyzer = {
			enterNode(node) {
				steps.push({ kind: 1, target: node, phase: 1, args: [node] });
			},
			leaveNode(node) {
				steps.push({ kind: 1, target: node, phase: 2, args: [node] });
			},
			emit(eventName, args) {
				steps.push({ kind: 2, target: eventName, args });
			},
		};
